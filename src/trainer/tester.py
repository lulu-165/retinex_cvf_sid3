...
class Test(BaseTrainer):
    ...
    def __init__(self, model, criterion, metric_ftns, optimizer, config, data_loader, test_data_loader,
        ...
        if os.path.isdir('../output/Nt') == False:
            os.makedirs('../output/Nt/')
    ...
    def _test_epoch(self, epoch, save=False):
        ...
        if save == True:
            ...
            os.makedirs('../output/Nt/' + str(epoch))
        for batch_idx, (target, input_noisy, input_GT, max_rgb, std) in enumerate(self.test_data_loader):
            ...
            clean, NiR, NdR, NiL, NdL, Nt, R, L, IR, IL = self.model(input_noisy,max_rgb)
            size = [Nt.shape[0], Nt.shape[1], Nt.shape[2] * Nt.shape[3]]
            Nt_normal = (Nt - torch.min(Nt.view(size), -1)[0].unsqueeze(-1).unsqueeze(-1)) / (
                        torch.max(Nt.view(size), -1)[0].unsqueeze(-1).unsqueeze(-1) -
                        torch.min(Nt.view(size), -1)[0].unsqueeze(-1).unsqueeze(-1))
            NdR_normal = (NdR - torch.min(NdR.view(size), -1)[0].unsqueeze(-1).unsqueeze(-1)) / (
            ...
            if save == True:
                for i in range(input_noisy.shape[0]):
                    ...
                    save_image(torch.clamp(Nt_normal[i, :, pad:-pad, pad:-pad], min=0, max=1).detach().cpu(),
                               '../output/Nt/' + str(epoch) + '/' + target['dir_idx'][i] + '.PNG')
                    save_image(torch.clamp(NdR_normal[i, :, pad:-pad, pad:-pad], min=0, max=1).detach().cpu(),
